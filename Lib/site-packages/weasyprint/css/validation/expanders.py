...
@expander('grid-templates')
@generic_expander('-columns', '-rows', '-areas')
def expand_grid_template(tokens, name):
    """Expand the ``grid-templates`` property."""
    yield from _expand_grid_template(tokens, name)


@expander('grid')
@generic_expander('-templates-columns', '-templates-rows', '-templates-areas',
...
def expand_grid(tokens, name):
    ...
    try:
        template = tuple(_expand_grid_template(tokens, 'grid-templates'))
    except InvalidValues:
        pass
    else:
        for key, value in template:
            yield f'-templates-{key.split("-")[-1]}', value
    ...
    yield f'-templates-{auto_track}s', [none]
    yield f'-templates-{non_auto_track}s', tuple(templates[non_auto_track])
    yield '-templates-areas', [none]
