...
@property('grid-templates-columns')
@property('grid-templates-rows')
def grid_template(tokens):
    """``grid-templates-columns`` and ``grid-templates-rows`` validation."""
    return_tokens = []
...
def grid_template_areas(tokens):
    """``grid-templates-areas`` property validation."""
    if len(tokens) == 1 and get_keyword(tokens[0]) == 'none':
        return 'none'
    grid_areas = []
    for token in tokens:
